...
SECRET_ACCESS_KEY = "CAR786"

# --- VALIDITY LOGIC (1 MONTH) ---
...
# --- 2. MAIN APP CONTENT (Unlocked) ---
    ...
    if loan_amt > 0:
        ...
        st.info("Enter values to see EMI preview.")
    ...
    if st.button("Generate Premium PDF Quotation"):
        if not cust_name or loan_amt == 0:
            ...
            c.drawCentredString(300, 805, "CAR MELA")
            ...
            c.drawRightString(540, 715, f"DATE: {current_time}")
            c.line(50, 685, 540, 685)
            ...
            c.setFont("Helvetica-Bold", 12)
            c.drawString(70, y, "Net Loan Amount")
            c.drawRightString(520, y, f"Rs. {loan_amt:,.2f}")
            y -= 25
            c.drawString(70, y, "Interest Rate")
            c.drawRightString(520, y, f"{roi}% ({int_type})")
            ...
            for m in [5, 10, 12, 15, 18, 24, 30, 36]:
                ...
                c.drawRightString(530, y, f"{emi*m:,.2f}")
                y -= 22
